...
class QueryLog(Base):
    ...
    reranker_model: Mapped[str | None] = mapped_column(String(150), nullable=True)
    reranker_top_k: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class Evaluation(Base):
    ...
    provider_generation_id: Mapped[str | None] = mapped_column(String(200), nullable=True)
    provider_usage_source: Mapped[str | None] = mapped_column(Text, nullable=True)
    latency_ms: Mapped[float] = mapped_column(Float)
    prompt_tokens: Mapped[int] = mapped_column(Integer, default=0)
    completion_tokens: Mapped[int] = mapped_column(Integer, default=0)
    total_tokens: Mapped[int] = mapped_column(Integer, default=0)
